...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	block, err := ec.BlockByNumber(context.Background(), nil, true)
	if err != nil {
...
func testGetBlock(t *testing.T, client *rpc.Client) {
	...
	// Get current block by number
	block, err := ec.BlockByNumber(context.Background(), new(big.Int).SetUint64(blockNumber), true)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	blockH, err := ec.BlockByHash(context.Background(), block.Hash(), true)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
